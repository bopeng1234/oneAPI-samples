...
void HschurSparseBlockMatrix::constructFromVertices(
  ...
  for (auto vL : verticesL) {
    if (vL->fixed) continue;

    indices.clear();
    for (const auto e : vL->edges) {
      const auto vP = e->poseVertex();
      if (!vP->fixed) indices.push_back(vP->iP);
    }
  ...
  std::sort(std::begin(blockpos), std::end(blockpos),
            [](const BlockPos& lhs, const BlockPos& rhs) {
  ...
  nnzPerRow_.setZero();
  for (int i = 0; i < nblocks_; i++) nnzPerRow_[blockpos[i].row]++;
  ...
  for (int blockRowId = 0; blockRowId < brows_; blockRowId++) {
    for (int i = browPtr_[blockRowId]; i < browPtr_[blockRowId + 1]; i++) {
      ...
      if (blockRowId == blockColId) {
        for (int dr = 0; dr < PDIM; dr++) nnzPerRow_[dstRowId0 + dr] += PDIM;
      } else {
        for (int dr = 0; dr < PDIM; dr++) nnzPerRow_[dstRowId0 + dr] += PDIM;
        for (int dc = 0; dc < PDIM; dc++) nnzPerRow_[dstColId0 + dc] += PDIM;
      }
    }
  }

  rowPtr_[0] = 0;
  for (int r = 0; r < drows; r++) rowPtr_[r + 1] = rowPtr_[r] + nnzPerRow_[r];

  for (int r = 0; r < drows; r++) nnzPerRow_[r] = rowPtr_[r];
