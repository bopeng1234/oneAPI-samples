...
static inline double get_duration(const time_point& from,
                                  const time_point& to) {
  return std::chrono::duration_cast<std::chrono::duration<double>>(to - from)
      .count();
}

static const char* profileItemString[8] = {
    "0: Initialize Optimizer",    "1: Build Structure",
    "2: Compute Error",           "3: Quadratic Form",
    "4: Schur Complement",        "5: Symbolic Decomposition",
    "6: Numerical Decomposition", "7: Update Solution"};
...
class DpcppBlockSolver {
 public:
  ...
  void initialize(const VertexMapP& vertexMapP, const VertexMapL& vertexMapL,
    ...
    if (!linearSolver_) linearSolver_ = SparseLinearSolver::create();

    // set profItems to zero
    profItems_.assign(PROF_ITEM_NUM, 0);

    const auto t1 = get_time_point();
    profItems_[PROF_ITEM_INITIALIZE] += get_duration(t0, t1);
    // std::printf("\n%-30s : %8.1f[msec]\n",
    // profileItemString[PROF_ITEM_INITIALIZE], 1e3 * get_duration(t0, t1));
  }

  void buildStructure() {
    const auto t0 = get_time_point();

    // allocate Hpp, Hll, Hpl device buffers
    d_H_.resize(numP_ * PDIM * PDIM + numL_ * LDIM * LDIM +
                nHplBlocks_ * PDIM * LDIM);
    d_Hpp_.map(numP_, d_H_.data());
    d_Hll_.map(numL_, d_H_.data() + numP_ * PDIM * PDIM);
    // d_Hpp_.resize(numP_);
    // d_Hll_.resize(numL_);

    // build Hpl block matrix structure
    d_Hpl_.resize(numP_, numL_);
    d_Hpl_.mapNonZeros(nHplBlocks_,
                       d_H_.data() + numP_ * PDIM * PDIM + numL_ * LDIM * LDIM);
    ...
    linearSolver_->initialize(d_Hpl_);
    ...
    // profileItemString[PROF_ITEM_BUILD_STRUCTURE], 1e3 * get_duration(t0,
    // t1)); std::printf("%-30s : %8.1f[msec]\n",
    // profileItemString[PROF_ITEM_DECOMP_SYMBOLIC], 1e3 * get_duration(t1,
    // t2));
  }
  ...
  double computeErrors() {
    ...
    const Scalar chi2D = gpu::computeActiveErrors(
        d_qs_, d_ts_, d_Xws_, d_measurements2D_, d_omegas2D_, d_edge2PL2D_,
        d_errors2D_, d_Xcs2D_, d_chi_.data());

    const Scalar chi3D = gpu::computeActiveErrors(
        d_qs_, d_ts_, d_Xws_, d_measurements3D_, d_omegas3D_, d_edge2PL3D_,
        d_errors3D_, d_Xcs3D_, d_chi_.data());

    const auto t1 = get_time_point();
    profItems_[PROF_ITEM_COMPUTE_ERROR] += get_duration(t0, t1);
    // std::printf("%-30s : %8.1f[msec]\n",
    // profileItemString[PROF_ITEM_COMPUTE_ERROR], 1e3 * get_duration(t0, t1));
  ...
  void buildSystem() {
    ...
    gpu::constructQuadraticForm(d_Xcs2D_, d_qs_, d_errors2D_, d_omegas2D_,
                                d_edge2PL2D_, d_edge2Hpl2D_, d_edgeFlags2D_,
                                d_Hpp_, d_bp_, d_Hll_, d_bl_, d_Hpl_);

    gpu::constructQuadraticForm(d_Xcs3D_, d_qs_, d_errors3D_, d_omegas3D_,
                                d_edge2PL3D_, d_edge2Hpl3D_, d_edgeFlags3D_,
                                d_Hpp_, d_bp_, d_Hll_, d_bl_, d_Hpl_);

    const auto t1 = get_time_point();
    profItems_[PROF_ITEM_BUILD_SYSTEM] += get_duration(t0, t1);
    // std::printf("%-30s : %8.1f[msec]\n",
    // profileItemString[PROF_ITEM_BUILD_SYSTEM], 1e3 * get_duration(t0, t1));
  }

  double maxDiagonal() {
    DeviceBuffer<Scalar> d_buffer(16);
    const Scalar maxP = gpu::maxDiagonal(d_Hpp_, d_buffer.data());
    const Scalar maxL = gpu::maxDiagonal(d_Hll_, d_buffer.data());
  ...
  bool solve() {
    ...
    ////////////////////////////////////////////////////////////////////////////////////
    // gpu::computeBschure(d_bp_, d_Hpl_, d_Hll_, d_bl_, d_bsc_, d_invHll_,
    // d_Hpl_invHll_); // output d_bsc_    6x1 block gpu::computeHschure(d_Hpp_,
    // d_Hpl_invHll_, d_Hpl_, d_HscMulBlockIds_, d_Hsc_);  // output d_Hsc_ 6x6
    // block
    ...
    // d_x_.fillZero();
    // const bool success = linearSolver_->solve(d_HscCSR_, d_bsc_.values(),
    // d_xp_.values());
    const bool success =
        linearSolver_->solve(d_H_.data(), d_b_.data(), d_x_.data());
    ...
    profItems_[PROF_ITEM_SCHUR_COMPLEMENT] +=
        (get_duration(t0, t1) + get_duration(t2, t3));
    profItems_[PROF_ITEM_DECOMP_NUMERICAL] += get_duration(t1, t2);
    // std::printf("%-30s : %8.1f[msec]\n",
    // profileItemString[PROF_ITEM_DECOMP_NUMERICAL], 1e3 * get_duration(t1,
    // t2));
  ...
  void update() {
    ...
    // std::printf("%-30s : %8.1f[msec]\n", profileItemString[PROF_ITEM_UPDATE],
    // 1e3 * get_duration(t0, t1));
  }
  ...
  void push() { d_solution_.copyTo(d_solutionBackup_); }

  void pop() { d_solutionBackup_.copyTo(d_solution_); }
...
/** @brief Implementation of DpcppBundleAdjustment.
 */
class DpcppBundleAdjustmentImpl : public DpcppBundleAdjustment {
 public:
  void addPoseVertex(VertexP* v) override { vertexMapP_.insert({v->id, v}); }
  ...
  VertexP* poseVertex(int id) const override { return vertexMapP_.at(id); }

  VertexL* landmarkVertex(int id) const override { return vertexMapL_.at(id); }

  void removePoseVertex(PoseVertex* v) override {
    auto it = vertexMapP_.find(v->id);
    if (it == std::end(vertexMapP_)) return;

    for (auto e : it->second->edges) removeEdge(e);
  ...
  void removeLandmarkVertex(LandmarkVertex* v) override {
    ...
    if (it == std::end(vertexMapL_)) return;
  ...
  void removeEdge(BaseEdge* e) override {
    ...
    if (vertexP->edges.count(e)) vertexP->edges.erase(e);

    auto vertexL = e->landmarkVertex();
    if (vertexL->edges.count(e)) vertexL->edges.erase(e);

    if (e->dim() == 2) {
      auto edge2D = reinterpret_cast<Edge2D*>(e);
      if (edges2D_.count(edge2D)) edges2D_.erase(edge2D);
    }

    if (e->dim() == 3) {
      auto edge3D = reinterpret_cast<Edge3D*>(e);
      if (edges3D_.count(edge3D)) edges3D_.erase(edge3D);
    }
  }

  void setCameraPrams(const CameraParams& camera) override { camera_ = camera; }

  size_t nposes() const override { return vertexMapP_.size(); }

  size_t nlandmarks() const override { return vertexMapL_.size(); }

  size_t nedges() const override { return edges2D_.size() + edges3D_.size(); }
  ...
  void optimize(int niterations) override {
    ...
    for (int iteration = 0; iteration < niterations; iteration++) {
      // std::printf("\n\niteration=%d \n", iteration);
      if (iteration == 0) solver_.buildStructure();
      ...
      if (q == maxq || rho <= 0 || !std::isfinite(lambda)) break;
    }
  ...
  const BatchStatistics& batchStatistics() const override { return stats_; }

  const TimeProfile& timeProfile() override { return timeProfile_; }

  ~DpcppBundleAdjustmentImpl() { clear(); }

 private:
  static inline double attenuation(double x) {
    return 1 - std::pow(2 * x - 1, 3);
  }
  static inline double clamp(double v, double lo, double hi) {
    return std::max(lo, std::min(v, hi));
  }

  DpcppBlockSolver solver_;
...
DpcppBundleAdjustment::Ptr DpcppBundleAdjustment::create() {
  return std::make_unique<DpcppBundleAdjustmentImpl>();
}

DpcppBundleAdjustment::~DpcppBundleAdjustment() {}
